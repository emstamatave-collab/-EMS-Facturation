...
@app.route('/health')
def health(): return {'ok': True, 'app':'EMS Facturation V10'}
...
def edit_client(cid):
    ...
    if request.method=='POST':
        ...
        existing_photos=con.execute("select count(*) n from client_attachments where client_id=? and mime like 'image/%'",(cid,)).fetchone()['n']
        new_photos=[f for f in request.files.getlist('attachments') if f and f.filename]
        available=max(0,10-existing_photos)
        for f in new_photos[:available]:
            saved=save_upload(f,f'client_{cid}')
            if saved: con.execute('insert into client_attachments(client_id,original_name,stored_name,mime) values(?,?,?,?)',(cid,*saved))
        con.commit(); con.close()
        if len(new_photos)>available:
            flash(f'{available} photo(s) ajoutée(s). Maximum : 10 photos par client.')
        else:
    ...
    photo_count=sum(1 for a in atts if (a['mime'] or '').startswith('image/'))
    att_html=''.join(
        f"<a href='/client/{cid}/attachment/{a['id']}' target='_blank' style='display:inline-block;margin:6px;text-decoration:none'><img src='/client/{cid}/attachment/{a['id']}' alt='Photo client' style='width:120px;height:90px;object-fit:cover;border-radius:8px;border:1px solid #ddd'></a>"
        for a in atts if (a['mime'] or '').startswith('image/')
    ) or '<span class="muted">Aucune photo.</span>'
    return page(f'''<div class="card"><h2>Modifier le client</h2><form method="post" enctype="multipart/form-data"><div class="grid"><div><label>Nom</label><input name="name" value="{r['name'] or ''}" required></div><div><label>Téléphone</label><input name="phone" value="{r['phone'] or ''}"></div><div><label>Email</label><input name="email" value="{r['email'] or ''}"></div><div><label>NIF</label><input name="nif" value="{r['nif'] or ''}"></div><div><label>STAT</label><input name="stat" value="{r['stat'] or ''}"></div><div><label>Adresse</label><textarea name="address">{r['address'] or ''}</textarea></div></div><p><label>Photos des machines du client ({photo_count}/10)</label><input type="file" name="attachments" multiple accept="image/*" capture="environment"></p><p class="muted">Jusqu'à 10 photos par client.</p><p><button>Enregistrer</button></p></form></div><div class="card"><h3>Photos des machines</h3>{att_html}</div>''')
