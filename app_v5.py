import sqlite3, os, re, hmac, uuid, mimetypes
...
def clients():
    ...
    return page(f'''<div class="card"><h2>Nouveau client</h2><form method="post" enctype="multipart/form-data"><div class="grid3"><div><label>Nom / société</label><input name="name" required></div><div><label>Téléphone</label><input name="phone"></div><div><label>Email</label><input name="email"></div><div><label>NIF</label><input name="nif"></div><div><label>STAT</label><input name="stat"></div><div><label>Adresse</label><textarea name="address"></textarea></div></div><p><label>Pièces jointes / photos du client</label><input type="file" name="attachments" multiple accept="image/*,application/pdf"><span class="small muted">Sur iPhone : appareil photo, photothèque ou fichiers.</span></p><p><button>Ajouter le client</button></p></form></div><div class="card"><div class="row"><h2 style="flex:1">Clients</h2><form><input class="search" name="q" value="{q}" placeholder="Rechercher nom ou NIF"></form></div><div class="tablewrap"><table><tr><th>Nom</th><th>NIF</th><th>STAT</th><th>Téléphone</th><th>Email</th></tr>{trs}</table></div></div>''')
...
def edit_client(cid):
    ...
    if request.method=='POST':
        con.execute('update clients set name=?,address=?,nif=?,stat=?,email=?,phone=? where id=?',(request.form['name'],request.form.get('address'),request.form.get('nif'),request.form.get('stat'),request.form.get('email'),request.form.get('phone'),cid))
        for f in request.files.getlist('attachments'):
            saved=save_upload(f,f'client_{cid}')
            if saved: con.execute('insert into client_attachments(client_id,original_name,stored_name,mime) values(?,?,?,?)',(cid,*saved))
        con.commit(); con.close(); flash('Client mis à jour.'); return redirect(url_for('edit_client',cid=cid))
    atts=con.execute('select * from client_attachments where client_id=? order by id desc',(cid,)).fetchall(); con.close()
    att_html=''.join(f"<div style='display:inline-block;margin:6px'><a class='btn2' href='/client/{cid}/attachment/{a['id']}' target='_blank'>{a['original_name']}</a></div>" for a in atts) or '<span class="muted">Aucune pièce jointe.</span>'
    return page(f'''<div class="card"><h2>Modifier le client</h2><form method="post" enctype="multipart/form-data"><div class="grid"><div><label>Nom</label><input name="name" value="{r['name'] or ''}" required></div><div><label>Téléphone</label><input name="phone" value="{r['phone'] or ''}"></div><div><label>Email</label><input name="email" value="{r['email'] or ''}"></div><div><label>NIF</label><input name="nif" value="{r['nif'] or ''}"></div><div><label>STAT</label><input name="stat" value="{r['stat'] or ''}"></div><div><label>Adresse</label><textarea name="address">{r['address'] or ''}</textarea></div></div><p><label>Ajouter des pièces jointes / photos</label><input type="file" name="attachments" multiple accept="image/*,application/pdf"></p><p><button>Enregistrer</button></p></form></div><div class="card"><h3>Pièces jointes du client</h3>{att_html}</div>''')
...
def new_document():
    ...
    if request.method=='POST':
        ...
        for d,q,p,disc in zip(request.form.getlist('description'),request.form.getlist('qty'),request.form.getlist('unit_price'),request.form.getlist('discount_pct')):
            if d.strip(): con.execute('insert into lines(doc_id,description,qty,unit_price,discount_pct) values(?,?,?,?,?)',(did,d,float(q or 0),float(p or 0),max(0,min(100,float(disc or 0)))))
...
def edit_document(doc_id):
    ...
    if request.method=='POST':
        ...
        for de,q,p,disc in zip(request.form.getlist('description'),request.form.getlist('qty'),request.form.getlist('unit_price'),request.form.getlist('discount_pct')):
            if de.strip(): con.execute('insert into lines(doc_id,description,qty,unit_price,discount_pct) values(?,?,?,?,?)',(doc_id,de,float(q or 0),float(p or 0),max(0,min(100,float(disc or 0)))))
        for f in request.files.getlist('photos'):
