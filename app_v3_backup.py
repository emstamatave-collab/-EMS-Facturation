...
from flask import Flask, render_template_string, request, redirect, url_for, send_file, flash, Response, jsonify
...
app=Flask(__name__)
app.secret_key='ems-v2-change-me'
...
<div class="wrap"><div class="nav"><a href="{{url_for('home')}}">Tableau de bord</a><a href="{{url_for('clients')}}">Clients</a><a href="{{url_for('documents')}}">Devis / Factures</a><a href="{{url_for('new_document')}}">+ Nouveau</a></div>
...
def pdf(doc_id):
    ...
    safe=re.sub(r'[^A-Za-z0-9_-]+','-',d['number']); out=BASE/f"{d['kind']}_{safe}.pdf"; c=canvas.Canvas(str(out),pagesize=A4); W,H=A4; L=14*mm; R=W-14*mm; y=H-12*mm
