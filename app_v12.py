        ...
        if UPLOAD_DIR.exists():
            for p in UPLOAD_DIR.rglob('*'):
                if p.is_file():
                    ...
                    file_count += 1
        z.writestr('SAUVEGARDE_EMS.txt', f"EMS Facturation V12\\nDate: {date.today().isoformat()}\\nBase: ems.db\\nFichiers/photos: {file_count}\\n")
...
@app.route('/health')
def health(): return {'ok': True, 'app':'EMS Facturation V12'}
...
def edit_client(cid):
    ...
    photos=con.execute('select * from client_machine_photos where client_id=? order by id desc',(cid,)).fetchall(); con.close()
    count=len(photos)
    gallery=''.join(
